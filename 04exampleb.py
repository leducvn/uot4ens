...
def plot_histogram(hour, a, e, cumulative=False, figtype=''):
   ...
   for iexp in range(nexp):
      lgd_file.write('S 0.13i '+symbol[iexp]+' 0.13i '+color[iexp]+' 0.3p 0.25i @~e@~='+str(epsilon)+', @~t@~='+str(tau[iexp])+'\n')
...
tau = [0.01,0.1,1.,10.]# mass regulation
nexp = len(tau)
q = 2.
...
output_file = output_dir+'/Figure04b.'+format
plot_histogram(time, 500*a, 500*e, cumulative=False, figtype='b')
