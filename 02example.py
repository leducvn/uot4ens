...
def plot_histogram(hour, a, e, b, cumulative=False, figtype=''):
   ...
   for imember in range(nmember):
      ...
      graph_file.close()
      os.system('psxy graph.txt -R -J -O -K -SC0.12 -Gdarkgreen >> graph.ps')
      os.system('psxy graph.txt -R -J -O -K -Wthickest,darkgreen >> graph.ps')
   # Mean
   graph_file=open('graph.txt', 'w')
   for itime in range(ntime):
      x = itime
      graph_file.write(str(x)+' '+str(e[itime])+'\n')
   graph_file.close()
   os.system('psxy graph.txt -R -J -O -K -SS0.12 -Gblue >> graph.ps')
   os.system('psxy graph.txt -R -J -O -K -Wthickest,blue >> graph.ps')
   # Barycenter
   graph_file=open('graph.txt', 'w')
   for itime in range(ntime):
      x = itime
      graph_file.write(str(x)+' '+str(b[itime])+'\n')
   graph_file.close()
   os.system('psxy graph.txt -R -J -O -K -ST0.12 -Gred >> graph.ps')
   os.system('psxy graph.txt -R -J -O -K -Wthickest,red >> graph.ps')

   lgd_file = open('graph.lgd','w')
   lgd_file.write('C black\n')
   lgd_file.write('S 0.13i C 0.13i darkgreen 0.3p 0.25i Members\n')
   lgd_file.write('S 0.13i S 0.13i blue 0.3p 0.25i Ensemble mean\n')
   lgd_file.write('S 0.13i T 0.13i red 0.3p 0.25i Expected mean\n')
   #lgd_file.write('S 0.13i A 0.13i brown 0.3p 0.25i Best member\n')
   lgd_file.close()
   os.system('pslegend -R -J -O -K -C0.05i/0.05i -D'+str(left)+'/'+str(top)+'/1.8i/1.0i/LT graph.lgd >> graph.ps')
   ...
   else: os.system('echo "'+str(0.5*(left+right))+' '+str(top)+' 20 0 4 CB Rainfall distribution" | pstext -R -J -O -K -Dj0./0.1 -Gblack -N >> graph.ps')
...
# Sinkhorn log: stable
sigma0 = 8; mean0 = 30; factor0 = 1.
#sigma1 = 5; mean1 = 70; factor1 = 1.
sigma1 = 8; mean1 = 60; factor1 = 1.
nmember = 2; ntime = 101
format = 'png'; density = '-density 100'

epsilon = 0.0001 # enstropic regulation (does not work if < 1e-4)
phi = 0.999999 #phi = tau/(tau+epsilon) tau is mass regulation
q = 2.
...
output_file = output_dir+'/Figure02a.'+format
plot_histogram(time, 500*a, 500*e, 500*b[1], cumulative=False, figtype='a')
shutil.move('graph.'+format, output_file)
for itime in range(1,ntime):
   a[:,itime] += a[:,itime-1]
   b[1,itime] += b[1,itime-1]
   e[itime] += e[itime-1]
output_file = output_dir+'/Figure02b.'+format
plot_histogram(time, 500*a, 500*e, 500*b[1], cumulative=True, figtype='b')
shutil.move('graph.'+format, output_file)
